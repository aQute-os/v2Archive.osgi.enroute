...
public class BundleMixinServerTest extends TestCase {
	...
	/**
	 * If only the hostname is provided (no slash), then the default action by
	 * the RedirectServlet will append "/index.html". This will serve the
	 * "debug" version of the WebServer INDEX page.
	 */
	...
	/**
	 * If the request is for an empty path ("/" only), then the default action
	 * by the RedirectServlet will append "index.html". This will serve the
	 * "debug" version of the WebServer INDEX page.
	 */
	...
	// Test backwards compatibility by accessing segregated content as mixin
	// content
	...
	/**
	 * If only the BSN is provided on the path (with slash), then the result
	 * should be a 200 because the redirect servlet will have intervened. This
	 * is the one case that is different from the BFS.
	 */
