 ...
 * into the public domain. Please read this file for an overview and then
 * MFRC522.cpp for comments on the specific functions. Search for "mf-rc522" on
 * ebay.com to purchase the MF-RC522 board. There are three hardware components
 * involved: 1) The micro controller: An Arduino 2) The PCD (short for Proximity
 * Coupling Device): NXP MFRC522 Contactless Reader IC 3) The PICC (short for
 * Proximity Integrated Circuit Card): A card or tag using the ISO 14443A
 * interface, eg Mifare or NTAG203. The microcontroller and card reader uses SPI
 * for communication. The protocol is described in the MFRC522 datasheet:
 * http://www.nxp.com/documents/data_sheet/MFRC522.pdf The card reader and the
 * tags communicate using a 13.56MHz electromagnetic field. The protocol is
 * defined in ISO/IEC 14443-3 Identification cards -- Contactless integrated
 * circuit cards -- Proximity cards -- Part 3: Initialization and
 * anticollision". A free version of the final draft can be found at
 * http://wg8.de/wg8n1496_17n3613_Ballot_FCD14443-3.pdf Details are found in
 * chapter 6, Type A – Initialization and anticollision. If only the PICC UID is
 * wanted, the above documents has all the needed information. To read and write
 * from MIFARE PICCs, the MIFARE protocol is used after the PICC has been
 * selected. The MIFARE Classic chips and protocol is described in the
 * datasheets: 1K: http://www.nxp.com/documents/data_sheet/MF1S503x.pdf 4K:
 ...
 * http://www.nxp.com/documents/short_data_sheet/MF0ICU2_SDS.pdf MIFARE Classic
 * 1K (MF1S503x): Has 16 sectors * 4 blocks/sector * 16 bytes/block = 1024
 * bytes. The blocks are numbered 0-63. Block 3 in each sector is the Sector
 * Trailer. See http://www.nxp.com/documents/data_sheet/MF1S503x.pdf sections
 * 8.6 and 8.7: Bytes 0-5: Key A Bytes 6-8: Access Bits Bytes 9: User data Bytes
 * 10-15: Key B (or user data) Block 0 is read-only manufacturer data. To access
 * a block, an authentication using a key from the block's sector must be
 * performed first. Example: To read from block 10, first authenticate using a
 * key from sector 3 (blocks 8-11). All keys are set to FFFFFFFFFFFFh at chip
 ...
 * operations) you need to change the sector trailer. Use PICC_SetAccessBits()
 * to calculate the bit patterns. MIFARE Classic 4K (MF1S703x): Has (32 sectors
 * * 4 blocks/sector + 8 sectors * 16 blocks/sector) * 16 bytes/block = 4096
 * bytes. The blocks are numbered 0-255. The last block in each sector is the
 * Sector Trailer like above. MIFARE Classic Mini (MF1 IC S20): Has 5 sectors *
 * 4 blocks/sector * 16 bytes/block = 320 bytes. The blocks are numbered 0-19.
 * The last block in each sector is the Sector Trailer like above. MIFARE
 * Ultralight (MF0ICU1): Has 16 pages of 4 bytes = 64 bytes. Pages 0 + 1 is used
 * for the 7-byte UID. Page 2 contains the last check digit for the UID, one
...
public class Rf522Impl {
	...
	//// // Reference values based on firmware version; taken from 16.1.1 in
	// spec.
	//// // Version 1.0
	...
	// // When using SPI all addresses are shifted one bit left in the "SPI
	// address byte" (section 8.1.2.3)
	// enum PCD_Register {
	...
	// CRCResultRegH (0x21), // shows the MSB and LSB values of the CRC
	// calculation
	// CRCResultRegL (0x22),
	...
	// GsNReg (0x27), // selects the conductance of the antenna driver pins TX1
	...
	// PCD_CalcCRC = 0x03, // activates the CRC coprocessor or performs a self
	// test
	...
	// PCD_MFAuthent = 0x0E, // performs the MIFARE standard authentication as a
	// reader
	...
	// RxGain_18dB_2 = 0x02 << 4, // 010b - 18 dB, it seems 010b is a duplicate
	// for 000b
	// RxGain_23dB_2 = 0x03 << 4, // 011b - 23 dB, it seems 011b is a duplicate
	// for 001b
	...
	// RxGain_18dB
	// RxGain_avg = 0x04 << 4, // 100b - 33 dB, average, convenience for
	// RxGain_33dB
	// RxGain_max = 0x07 << 4 // 111b - 48 dB, maximum, convenience for
	// RxGain_48dB
	// };
	//
	...
	// authenticated sector of the PICC. Called "COMPATIBILITY WRITE" for MIFARE
	// Ultralight.
	// PICC_CMD_MF_DECREMENT = 0xC0, // Decrements the contents of a block and
	// stores the result in the internal data register.
	...
	// STATUS_INTERNAL_ERROR = 6, // Internal error in the code. Should not
	// happen ;-)
	...
	// byte *backLen, byte *validBits = NULL, byte rxAlign = 0, bool checkCRC =
	// false);
	// byte PCD_CommunicateWithPICC(byte command, byte waitIRq, byte *sendData,
	// byte sendLen, byte *backData = NULL, byte *backLen = NULL, byte
	// *validBits = NULL, byte rxAlign = 0, bool checkCRC = false);
	...
	// /////////////////////////////////////////////////////////////////////////////////////
	// byte PCD_Authenticate(byte command, byte blockAddr, MIFARE_Key *key, Uid
	// *uid);
	...
	// // Support functions
	// /////////////////////////////////////////////////////////////////////////////////////
	// byte PCD_MIFARE_Transceive(byte *sendData, byte sendLen, bool
	// acceptTimeout = false);
	// // old function used too much memory, now name moved to flash; if you
	// need char, copy from flash to memory
	...
	// void PICC_DumpMifareClassicToSerial(Uid *uid, byte piccType, MIFARE_Key
	// *key);
	// void PICC_DumpMifareClassicSectorToSerial(Uid *uid, MIFARE_Key *key, byte
	// sector);
	// void PICC_DumpMifareUltralightToSerial();
	// void MIFARE_SetAccessBits(byte *accessBitBuffer, byte g0, byte g1, byte
	// g2, byte g3);
	...
	//
	// private:
	// byte _chipSelectPin; // Arduino pin connected to MFRC522's SPI slave
	...
	//
	// #endif /*
	// * MFRC522.cpp - Library to use ARDUINO RFID MODULE KIT 13.56 MHZ WITH
	// TAGS SPI W AND R BY COOQROBOT.
	// * NOTE: Please also check the comments in MFRC522.h - they provide useful
	// hints and background information.
	...
	// */
	// MFRC522::MFRC522( byte chipSelectPin, ///< Arduino pin connected to
	...
	// */
	// void MFRC522::PCD_WriteRegister( byte reg, ///< The register to write to.
	// One of the PCD_Register enums.
	// byte value ///< The value to write.
	// ) {
	// digitalWrite(_chipSelectPin, LOW); // Select slave
	// SPI.transfer(reg & 0x7E); // MSB == 0 is for writing. LSB is not used in
	...
	// * The interface is described in the datasheet section 8.1.2.
	// */
	// byte MFRC522::PCD_ReadRegister( byte reg ///< The register to read from.
	// One of the PCD_Register enums.
	// ) {
	// byte value;
	// digitalWrite(_chipSelectPin, LOW); // Select slave
	// SPI.transfer(0x80 | (reg & 0x7E)); // MSB == 1 is for reading. LSB is not
	// used in address. Datasheet section 8.1.2.3.
	...
	//
	// /**
	// * Reads a number of bytes from the specified register in the MFRC522
	// chip.
	// * The interface is described in the datasheet section 8.1.2.
	// */
	// void MFRC522::PCD_ReadRegister( byte reg, ///< The register to read from.
	// One of the PCD_Register enums.
	...
	// bytes from register."));
	// byte address = 0x80 | (reg & 0x7E); // MSB == 1 is for reading. LSB is
	// not used in address. Datasheet section 8.1.2.3.
	// byte index = 0; // Index in values array.
	// digitalWrite(_chipSelectPin, LOW); // Select slave
	// count--; // One read is performed outside of the loop
	// SPI.transfer(address); // Tell MFRC522 which address we want to read
	// while (index < count) {
	// if (index == 0 && rxAlign) { // Only update bit positions rxAlign..7 in
	// values[0]
	...
	// }
	// else { // Normal case
	// values[index] = SPI.transfer(address); // Read value and tell that we
	// want to read the same address again.
	// }
	// index++;
	// }
	// values[index] = SPI.transfer(0); // Read the final byte. Send 0 to stop
	// reading.
	// digitalWrite(_chipSelectPin, HIGH); // Release slave again
	// } // End PCD_ReadRegister()
	//
	...
	// */
	// void MFRC522::PCD_SetRegisterBitMask( byte reg, ///< The register to
	// update. One of the PCD_Register enums.
	...
	// */
	// void MFRC522::PCD_ClearRegisterBitMask( byte reg, ///< The register to
	// update. One of the PCD_Register enums.
	...
	// */
	// byte MFRC522::PCD_CalculateCRC( byte *data, ///< In: Pointer to the data
	// to transfer to the FIFO for CRC calculation.
	...
	// bit
	// PCD_SetRegisterBitMask(FIFOLevelReg, 0x80); // FlushBuffer = 1, FIFO
	// initialization
	...
	// byte n;
	// while (1) {
	// n = PCD_ReadRegister(DivIrqReg); // DivIrqReg[7..0] bits are: Set2
	...
	// this one after 89ms. Communication with the MFRC522 might be down.
	...
	// TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer
	// period of 25�s.
	// PCD_WriteRegister(TReloadRegH, 0x03); // Reload timer with 0x3E8 = 1000,
	...
	// complete.
	// // But the MFRC522 might have been in soft power-down mode (triggered by
	// bit 4 of CommandReg)
	// // Section 8.8.2 in the datasheet says the oscillator start-up time is
	// the start up time of the crystal + 37,74�s. Let us be generous: 50ms.
	// delay(50);
	...
	// http://www.nxp.com/documents/data_sheet/MFRC522.pdf
	// * NOTE: Return value scrubbed with (0x07<<4)=01110000b as RCFfgReg may
	// use reserved bits.
	// *
	...
	//
	// /**
	// * Set the MFRC522 Receiver Gain (RxGain) to value specified by given
	// mask.
	// * See 9.3.3.6 / table 98 in
	// http://www.nxp.com/documents/data_sheet/MFRC522.pdf
	...
	// PCD_ClearRegisterBitMask(RFCfgReg, (0x07<<4)); // clear needed to allow
	// 000 pattern
	// PCD_SetRegisterBitMask(RFCfgReg, mask & (0x07<<4)); // only set
	// RxGain[2:0] bits
	// }
	...
	// PCD_WriteRegister(FIFODataReg, 25, ZEROES); // write 25 bytes of 00h to
	// FIFO
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PCD_TransceiveData( byte *sendData, ///< Pointer to the
	// data to transfer to the FIFO.
	// byte sendLen, ///< Number of bytes to transfer to the FIFO.
	// byte *backData, ///< NULL or pointer to buffer if data should be read
	// back after executing the command.
	// byte *backLen, ///< In: Max number of bytes to write to *backData. Out:
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PCD_CommunicateWithPICC( byte command, ///< The command to
	// execute. One of the PCD_Command enums.
	...
	// byte bitFraming = (rxAlign << 4) + txLastBits; // RxAlign =
	...
	// PCD_SetRegisterBitMask(FIFOLevelReg, 0x80); // FlushBuffer = 1, FIFO
	// initialization
	// PCD_WriteRegister(FIFODataReg, sendLen, sendData); // Write sendData to
	// the FIFO
	// PCD_WriteRegister(BitFramingReg, bitFraming); // Bit adjustments
	// PCD_WriteRegister(CommandReg, command); // Execute the command
	// if (command == PCD_Transceive) {
	// PCD_SetRegisterBitMask(BitFramingReg, 0x80); // StartSend=1, transmission
	// of data starts
	// }
	//
	...
	// i = 2000;
	// while (1) {
	// n = PCD_ReadRegister(ComIrqReg); // ComIrqReg[7..0] bits are: Set1 TxIRq
	// RxIRq IdleIRq HiAlertIRq LoAlertIRq ErrIRq TimerIRq
	...
	// will eventually terminate on this one after 35.7ms. Communication with
	...
	// byte errorRegValue = PCD_ReadRegister(ErrorReg); // ErrorReg[7..0] bits
	...
	// PCD_ReadRegister(FIFODataReg, n, backData, rxAlign); // Get received data
	// from FIFO
	// _validBits = PCD_ReadRegister(ControlReg) & 0x07; // RxLastBits[2:0]
	...
	// return n;
	// }
	// if ((backData[*backLen - 2] != controlBuffer[0]) || (backData[*backLen -
	// 1] != controlBuffer[1])) {
	// return STATUS_CRC_WRONG;
	// }
	// }
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PICC_RequestA(byte *bufferATQA, ///< The buffer to store
	// the ATQA (Answer to request) in
	...
	//
	// /**
	// * Transmits a Wake-UP command, Type A. Invites PICCs in state IDLE and
	// HALT to go to READY(*) and prepare for anticollision or selection. 7 bit
	// frame.
	// * Beware: When two PICCs are in the field at the same time I often get
	// STATUS_TIMEOUT - probably due do bad antenna design.
	// *
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PICC_WakeupA( byte *bufferATQA, ///< The buffer to store
	// the ATQA (Answer to request) in
	// byte *bufferSize ///< Buffer size, at least two bytes. Also number of
	// bytes returned if STATUS_OK.
	// ) {
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PICC_REQA_or_WUPA( byte command, ///< The command to send -
	// PICC_CMD_REQA or PICC_CMD_WUPA
	// byte *bufferATQA, ///< The buffer to store the ATQA (Answer to request)
	// in
	// byte *bufferSize ///< Buffer size, at least two bytes. Also number of
	// bytes returned if STATUS_OK.
	// ) {
	...
	// byte status;
	//
	// if (bufferATQA == NULL || *bufferSize < 2) { // The ATQA response is 2
	// bytes long.
	// return STATUS_NO_ROOM;
	// }
	// PCD_ClearRegisterBitMask(CollReg, 0x80); // ValuesAfterColl=1 => Bits
	...
	// UIDs two or three iterations are used:
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PICC_Select( Uid *uid, ///< Pointer to Uid struct. Normally
	// output, but can also be used to supply a known UID.
	...
	// current Cascade Level.
	// byte buffer[9]; // The SELECT/ANTICOLLISION commands uses a 7 byte
	...
	// first bit received.
	// byte txLastBits; // Used in BitFramingReg. The number of valid bits in
	...
	// // Byte 1: NVB Number of Valid Bits (in complete command, not just the
	// UID): High nibble: complete bytes, Low nibble: Extra bits.
	...
	// // Byte 7: CRC_A
	// // Byte 8: CRC_A
	// // The BCC and CRC_A is only transmitted if we know all the UID bits of
	...
	// uidIndex = 0;
	// useCascadeTag = validBits && uid->size > 4; // When we know that the UID
	...
	// uidIndex = 3;
	// useCascadeTag = validBits && uid->size > 7; // When we know that the UID
	...
	// }
	// byte bytesToCopy = currentLevelKnownBits / 8 + (currentLevelKnownBits % 8
	// ? 1 : 0); // The number of bytes needed to represent the known bits for
	// this level.
	// if (bytesToCopy) {
	// byte maxBytes = useCascadeTag ? 3 : 4; // Max 4 bytes in each Cascade
	// Level. Only 3 left if we use the Cascade Tag
	...
	// count = currentLevelKnownBits / 8; // Number of whole bytes in the UID
	// part.
	...
	// PCD_WriteRegister(BitFramingReg, (rxAlign << 4) + txLastBits); // RxAlign
	// = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]
	//
	...
	// collision.
	// result = PCD_ReadRegister(CollReg); // CollReg[7..0] bits are:
	// ValuesAfterColl reserved CollPosNotValid CollPos[4:0]
	// if (result & 0x20) { // CollPosNotValid
	// return STATUS_COLLISION; // Without a valid collision position we cannot
	// continue
	// }
	...
	// index = 1 + (currentLevelKnownBits / 8) + (count ? 1 : 0); // First byte
	// is index 0.
	...
	// if (responseLength != 3 || txLastBits != 0) { // SAK must be exactly 24
	// bits (1 byte + CRC_A).
	// return STATUS_ERROR;
	// }
	// // Verify CRC_A - do our own calculation and store the control in
	...
	// 10.3.1.9 and http://www.nxp.com/documents/data_sheet/MF1S503x.pdf section
	// 10.1.
	// * For use with MIFARE Classic PICCs.
	// * The PICC must be selected - ie in state ACTIVE(*) - before calling this
	// function.
	// * Remember to call PCD_StopCrypto1() after communicating with the
	...
	// */
	// byte MFRC522::PCD_Authenticate(byte command, ///< PICC_CMD_MF_AUTH_KEY_A
	// or PICC_CMD_MF_AUTH_KEY_B
	// byte blockAddr, ///< The block number. See numbering in the comments in
	// the .h file.
	// MIFARE_Key *key, ///< Pointer to the Crypteo1 key to use (6 bytes)
	// Uid *uid ///< Pointer to Uid struct. The first 4 bytes of the UID is
	// used.
	// ) {
	...
	// returned.
	// * A roll-back is implemented: If blockAddr is 0Eh, then the contents of
	// pages 0Eh, 0Fh, 00h and 01h are returned.
	// *
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Read( byte blockAddr, ///< MIFARE Classic: The block
	// (0-0xff) number. MIFARE Ultralight: The first page to return data from.
	// byte *buffer, ///< The buffer to store the data in
	// byte *bufferSize ///< Buffer size, at least 18 bytes. Also number of
	// bytes returned if STATUS_OK.
	// ) {
	// byte result;
	//
	...
	// least significant 4 bytes (bytes 0 to 3)
	// * are written to the specified address. It is recommended to set the
	// remaining bytes 04h to 0Fh to all logic 0.
	// * *
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Write( byte blockAddr, ///< MIFARE Classic: The
	// block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
	...
	// that the response is MF_ACK.
	// if (result != STATUS_OK) {
	// return result;
	// }
	//
	// // Step 2: Transfer the data
	// result = PCD_MIFARE_Transceive(buffer, bufferSize); // Adds CRC_A and
	// checks that the response is MF_ACK.
	// if (result != STATUS_OK) {
	// return result;
	// }
	//
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Ultralight_Write( byte page, ///< The page (2-15) to
	// write to.
	// byte *buffer, ///< The 4 bytes to write to the PICC
	// byte bufferSize ///< Buffer size, must be at least 4 bytes. Exactly 4
	// bytes are written.
	// ) {
	// byte result;
	//
	...
	//
	// // Perform the write
	// result = PCD_MIFARE_Transceive(cmdBuffer, 6); // Adds CRC_A and checks
	// that the response is MF_ACK.
	// if (result != STATUS_OK) {
	// return result;
	// }
	...
	//
	// /**
	// * MIFARE Decrement subtracts the delta from the value of the addressed
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Decrement( byte blockAddr, ///< The block (0-0xff)
	// number.
	// long delta ///< This number is subtracted from the value of block
	// blockAddr.
	// ) {
	...
	//
	// /**
	// * MIFARE Increment adds the delta to the value of the addressed block,
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Increment( byte blockAddr, ///< The block (0-0xff)
	// number.
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Restore( byte blockAddr ///< The block (0-0xff)
	// number.
	// ) {
	...
	//
	// // Step 2: Transfer the data
	// result = PCD_MIFARE_Transceive( (byte *)&data, 4, true); // Adds CRC_A
	// and accept timeout as success.
	// if (result != STATUS_OK) {
	// return result;
	// }
	//
	...
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::MIFARE_Transfer( byte blockAddr ///< The block (0-0xff)
	// number.
	// ) {
	// byte result;
	// byte cmdBuffer[2]; // We only need room for 2 bytes.
	...
	// * Adds CRC_A, executes the Transceive command and checks that the
	// response is MF_ACK or a timeout.
	// *
	// * @return STATUS_OK on success, STATUS_??? otherwise.
	// */
	// byte MFRC522::PCD_MIFARE_Transceive( byte *sendData, ///< Pointer to the
	// data to transfer to the FIFO. Do NOT include the CRC_A.
	...
	// */
	// const __FlashStringHelper *MFRC522::GetStatusCodeName(byte code ///< One
	// of the StatusCode enums.
	// ) {
	// switch (code) {
	...
	// case STATUS_INTERNAL_ERROR: return F("Internal error in the code. Should
	// not happen."); break;
	...
	// * @return PICC_Type
	// */
	// byte MFRC522::PICC_GetType(byte sak ///< The SAK byte returned from
	// PICC_Select().
	// ) {
	...
	// One of the PICC_Type enums.
	// ) {
	// switch (piccType) {
	// case PICC_TYPE_ISO_14443_4: return F("PICC compliant with ISO/IEC
	// 14443-4"); break;
	// case PICC_TYPE_ISO_18092: return F("PICC compliant with ISO/IEC 18092
	// (NFC)");break;
	...
	// case PICC_TYPE_NOT_COMPLETE: return F("SAK indicates UID is not
	// complete."); break;
	...
	// */
	// void MFRC522::PICC_DumpToSerial(Uid *uid ///< Pointer to Uid struct
	// returned from a successful PICC_Select().
	// ) {
	// MIFARE_Key key;
	//
	// // UID
	...
	// * On success the PICC is halted after dumping the data.
	// */
	// void MFRC522::PICC_DumpMifareClassicToSerial( Uid *uid, ///< Pointer to
	// Uid struct returned from a successful PICC_Select().
	...
	// */
	// void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid, ///< Pointer
	// to Uid struct returned from a successful PICC_Select().
	...
	// c2_ = buffer[6] >> 4;
	// c3_ = buffer[7] & 0xF;
	// invertedError = (c1 != (~c1_ & 0xF)) || (c2 != (~c2_ & 0xF)) || (c3 !=
	// (~c3_ & 0xF));
	...
	// }
	//
	// if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector
	// trailer, a value block
	...
	// pages.
	// for (byte page = 0; page < 16; page +=4) { // Read returns data for 4
	// pages at a time.
	// // Read pages
	// byteCount = sizeof(buffer);
	...
	//
	// /**
	// * Calculates the bit pattern needed for the specified access bits. In the
	// [C1 C2 C3] tupples C1 is MSB (=4) and C3 is LSB (=1).
	// */
	// void MFRC522::MIFARE_SetAccessBits( byte *accessBitBuffer, ///< Pointer
	// to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
	...
	// ) {
	// byte c1 = ((g3 & 4) << 1) | ((g2 & 4) << 0) | ((g1 & 4) >> 1) | ((g0 & 4)
	// >> 2);
	// byte c2 = ((g3 & 2) << 2) | ((g2 & 2) << 1) | ((g1 & 2) << 0) | ((g0 & 2)
	// >> 1);
	...
	// * Mifare cards to allow writing to sector 0, where the card UID is
	// stored.
	// *
	// * Note that you do not need to have selected the card through REQA or
	// WUPA,
	// * this sequence works immediately when the card is in the reader
	// vicinity.
	...
	// if (logErrors) {
	// Serial.println(F("Failed to authenticate to card for reading, could not
	// set UID: "));
	// Serial.println(GetStatusCodeName(status));
	// }
	// return false;
	...
	// Serial.print(F("MIFARE_Read() failed: "));
	// Serial.println(GetStatusCodeName(status));
	// Serial.println(F("Are you sure your KEY A for sector 0 is
	// 0xFFFFFFFFFFFF?"));
	// }
	// return false;
	// }
	...
	//
	// /**
	// * Resets entire sector 0 to zeroes, so the card can be read again by
	// readers.
	// */
