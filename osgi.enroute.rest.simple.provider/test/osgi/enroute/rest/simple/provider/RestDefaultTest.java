...
public class RestDefaultTest extends TestCase {
	...
	// try {
	// URL url = new
	// URL("http://localhost:8080/rest/foo?default=default&multiple=1&multiple=2");
	...
	// return rq.FOO_HEADER() + "|" + rq.FOO$2CHEADER() + "|" +
	// rq.FOO$5FHEADER();
	// }
	// }
	//
	...
	//
	// Object[] array = foo.parameters.stream().map(p ->
	// p.name).sorted().toArray();
	...
	// jsonCodec.addHandler(PrimitiveType.class, new
	// EnumHandler(PrimitiveType.class) {
	//
	// @Override
	...
	// @Description("An example POST call with payload as argument and one
	// string argument")
	...
	// @Description("An example PUT call with payload as argument and one string
	// argument")
	...
	// jsonCodec.dec().from(url.openStream()).get(OpenAPIObject.class);
