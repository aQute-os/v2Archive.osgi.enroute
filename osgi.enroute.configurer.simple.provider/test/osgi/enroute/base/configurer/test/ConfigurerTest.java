...
public class ConfigurerTest extends TestCase {
	...
	public void testSimple() throws Exception {
		...
		lp.getBundleContext()
			.registerService(ConfigurationListener.class, new ConfigurationListener() {
		...
		//
		// Now we make a new bundle that will set a new port for the singleton
		// (10)
	...
	private void checkConfigs(int port, int factories) throws IOException, InvalidSyntaxException {
		...
		assertNotNull(c);
		assertEquals(port, c.getProperties()
			.get("port"));
	...
	private Bundle doBundleFor(String resource) throws Exception, BundleException {
		try (Builder b = new Builder()) {
			...
			b.build();
			b.getJar()
				.putResource("configuration/configuration.json", Resource.fromURL(getClass().getResource(resource)));

			Bundle a = lp.getBundleContext()
				.installBundle("test", new JarResource(b.getJar()).openInputStream());
			a.start();
			return a;
		}
	}
